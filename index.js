				...
				switch (cmdBaseArgs[0]) {
					...
					case 'update': {
						...
						let iterate = () => {
							...
							if (packageType !== null && packageType !== 'user') { // If so, installIds will be [packageId]
...
function _onGotError(err) {
	if (!err.response) {
		...
		switch (err.response.statusCode) {
			case 404:
				log(`Server error: 404 Not Found`);
				break;
			case 403:
				log(`Server error: 403 Forbidden`);
				break;
			case 500:
				log(`Server error: 500 Internal Server Error`, err.response);
				break;
		}
		process.exit();
	}
...
function getDirectoryFiles(dir, relDir = '') {
	let results = [];
	let list = fs.readdirSync(dir);
	...
	for (let fileName of list) {
		...
		let stat = fs.statSync(filePath);
		if (stat.isDirectory()) {
			let subPaths = getDirectoryFiles(filePath, relDir + fileName + '/');
